...
#define QukanPopupDeprecated(instead) __attribute__((deprecated(instead)))
...
#ifdef DEBUG
#define QukanPopupLog(FORMAT, ...) fprintf(stderr,"%s:%d\t%s\n",[[[NSString stringWithUTF8String:__FILE__] lastPathComponent] UTF8String], __LINE__, [[NSString stringWithFormat:FORMAT, ##__VA_ARGS__] UTF8String]);
#else
#define QukanPopupLog(...)
#endif
...
UIKIT_EXTERN NSString *const QukanPopupWaitDataDurationArriveNotification;
UIKIT_EXTERN NSString *const QukanPopupDetailPageWillShowNotification;
UIKIT_EXTERN NSString *const QukanPopupDetailPageShowFinishNotification;
/** GIFImageCycleOnce = YES(GIF不循环)时, GIF动图播放完成通知 */
UIKIT_EXTERN NSString *const QukanPopupGIFImageCycleOnceFinishNotification;

UIKIT_EXTERN BOOL QukanPopupPrefersHomeIndicatorAutoHidden;
