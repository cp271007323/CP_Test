...
@implementation QukanPopupCache
...
+(BOOL)saveImageData:(NSData *)data imageURL:(NSURL *)url{
    NSString *path = [NSString stringWithFormat:@"%@/%@",[self JiuAilaunchCachePath],[self keyWithURL:url]];
    if (data) {
        BOOL result = [[NSFileManager defaultManager] createFileAtPath:path contents:data attributes:nil];
        if (!result) QukanPopupLog(@"cache file error for URL: %@", url);
        return result;
    }
    return NO;
}
...
//    NSString *savePath = [[self JiuAilaunchCachePath] stringByAppendingPathComponent:[self videoNameWithURL:url]];
...
+ (NSString *)JiuAilaunchCachePath{
    NSString *path =[NSHomeDirectory() stringByAppendingPathComponent:@"Library/QukanPopupCache"];
...
+(NSString *)imagePathWithURL:(NSURL *)url{
    if(url==nil) return nil;
    return [[self JiuAilaunchCachePath] stringByAppendingPathComponent:[self keyWithURL:url]];
}

//+(NSString *)videoPathWithURL:(NSURL *)url{
//    if(url==nil) return nil;
//    return [[self JiuAilaunchCachePath] stringByAppendingPathComponent:[self videoNameWithURL:url]];
//}
//
//+(NSString *)videoPathWithFileName:(NSString *)videoFileName{
//    if(videoFileName.length==0) return nil;
//    return [[self JiuAilaunchCachePath] stringByAppendingPathComponent:[self videoNameWithURL:[NSURL URLWithString:videoFileName]]];
//}
...
+(void)clearDiskCache{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSFileManager *fileManager = [NSFileManager defaultManager];
        NSString *path = [self JiuAilaunchCachePath];
        [fileManager removeItemAtPath:path error:nil];
        [self checkDirectory:[self JiuAilaunchCachePath]];
    });
}
...
+(void)clearDiskCacheExceptImageUrlArray:(NSArray<NSURL *> *)exceptImageUrlArray{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSArray *allFilePaths = [self allFilePathWithDirectoryPath:[self JiuAilaunchCachePath]];
        NSArray *exceptImagePaths = [self filePathsWithFileUrlArray:exceptImageUrlArray type:NO];
        [allFilePaths enumerateObjectsUsingBlock:^(NSString *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if(![exceptImagePaths containsObject:obj] && !XHIStypeWithPath(obj)){
                [[NSFileManager defaultManager] removeItemAtPath:obj error:nil];
            }
        }];
        QukanPopupLog(@"allFilePath = %@",allFilePaths);
    });
}
...
//    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
//        NSArray *allFilePaths = [self allFilePathWithDirectoryPath:[self JiuAilaunchCachePath]];
//        NSArray *exceptVideoPaths = [self filePathsWithFileUrlArray:exceptVideoUrlArray type:YES];
//        [allFilePaths enumerateObjectsUsingBlock:^(NSString *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
//            if(![exceptVideoPaths containsObject:obj] && XHIStypeWithPath(obj)){
//                [[NSFileManager defaultManager] removeItemAtPath:obj error:nil];
//            }
//        }];
//        QukanPopupLog(@"allFilePath = %@",allFilePaths);
...
+(float)diskCacheSize{
    NSString *directoryPath = [self JiuAilaunchCachePath];
    BOOL isDir = NO;
...
+(NSArray *)filePathsWithFileUrlArray:(NSArray <NSURL *> *)fileUrlArray type:(BOOL)type{
...
+ (void)createBaseDirectoryAtPath:(NSString *)path {
    ...
    if (error) {
        QukanPopupLog(@"create cache directory failed, error = %@", error);
    } else {
        [self addDoNotBackupAttribute:path];
    }
    QukanPopupLog(@"QukanPopupCachePath = %@",path);
}
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        QukanPopupLog(@"error to set do not backup attribute, error = %@", error);
    }
}
