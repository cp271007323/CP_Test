...
@implementation QukanBMChineseSortSetting

+(QukanBMChineseSortSetting*)share{
    static QukanBMChineseSortSetting * singleton = nil ;
    if (singleton == nil) {
        singleton = [[QukanBMChineseSortSetting alloc] init];
...
@interface QukanBMChineseSortModel : NSObject
//用将需要排序的对象封装在QukanBMChineseSort对象中，包含排序的字符串，对象，首字母三个属性
//进行比较的字符串，
...
@implementation QukanBMChineseSortModel
@end
...
- (NSComparisonResult)mySort:(NSString *)str {
    NSString*s = [QukanBMChineseSortSetting share].specialCharSectionTitle;
    BOOL b = [QukanBMChineseSortSetting share].specialCharPositionIsFront;
...
@implementation QukanBMChineseSort
#pragma mark ============== tools ==================
...
+(void)sortAndGroup:(NSArray*)objectArray
    ...
    if (key == nil) {
        if (![obj isKindOfClass:NSString.class]) {
            [QukanBMChineseSort logMsg:@"数组内元素不是字符串类型,如果是对象类型，请传key"];
    ...
    if (!containKey) {
        [QukanBMChineseSort logMsg:@"数组内元素未包含指定属性"];
        finish(NO,nil,nil,nil);
        return;
    }
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        [QukanBMChineseSort logMsg:@""];
        [QukanBMChineseSort logMsg:[NSString stringWithFormat:@"转拼音用时：\t %f s", (state1-start)]];

        //根据QukanBMChineseSortModel的pinYin字段 升序 排列
        ...
        [QukanBMChineseSort logMsg:[NSString stringWithFormat:@"排序用时：\t %f s", (state2-state1)]];
        ...
        for (QukanBMChineseSortModel* object in sortModelArray) {
            NSString *firstLetter = [object.pinYin substringToIndex:1];
            id obj = object.object;
            [unSortedArr addObject:obj];
            //不同
            if(![lastTitle isEqualToString:firstLetter] && firstLetter){
        ...
        [QukanBMChineseSort logMsg:[NSString stringWithFormat:@"分组用时：\t %f s", (state3-state2)]];
        [QukanBMChineseSort logMsg:[NSString stringWithFormat:@"QukanBMChineseSort 排序总计用时：\t %f s", (state3-start)]];
...
//将对象 转为 QukanBMChineseSortModel
+(QukanBMChineseSortModel*)getModelWithObj:(id)obj key:(NSString*)key{
    QukanBMChineseSortModel *model = [[QukanBMChineseSortModel alloc]init];
    model.object = obj;
    if (!key) {
        model.string = obj;
    }else{
    ...
    if(model.string == nil || model.string.length == 0){
        model.string = QukanBMChineseSortSetting.share.specialCharSectionTitle;
    }else{
        //过滤 ignoreModelWithPrefix
        NSString *prefix = [model.string substringToIndex:1];
        if (![QukanBMChineseSortSetting.share.ignoreModelWithPrefix containsString:prefix]) {
            //是否将字母与汉字拼音一起排序？？？？ （暂不考虑）
            //获取拼音首字母
...
+(void)logMsg:(NSString*)msg{
    if (QukanBMChineseSortSetting.share.logEable == YES) {
        NSLog(@"------------------");
        NSLog(@"%@", msg);
        NSLog(@"------------------");
    }
...
+ (NSString *)getFirstLetter:(NSString *)chinese{
    ...
    //吧多音字先替换
    [QukanBMChineseSortSetting.share.polyphoneMapping enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        newChinese = [newChinese stringByReplacingOccurrencesOfString:key withString:obj];
    }];

    NSMutableString *result = [NSMutableString string];

    if (QukanBMChineseSortSetting.share.sortMode == 1) {
        //此处 对整个字符串转中文 而不单个字一次转 因为CFStringTransform太耗时 而这个时间又与字个数关系不大，所以尽量减少调用次数 以减少时间
        NSArray *wordArr = [[QukanBMChineseSort transformChinese:newChinese] componentsSeparatedByString:@" "];
    ...
    if ([upperCaseStr characterAtIndex:0] >= 'A' && [upperCaseStr characterAtIndex:0] <= 'Z') {
        ...
        return QukanBMChineseSortSetting.share.specialCharSectionTitle;
    }
}
