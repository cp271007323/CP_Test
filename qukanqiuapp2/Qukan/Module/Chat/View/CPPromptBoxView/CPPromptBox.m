...
-(void)upDataFrame{
    ...
    self.bounds = CGRectMake(0, 0, self.option.width, count * CPPBHeight + self.option.triangleHeight);
    _boxTableView.frame = CGRectMake(0, tableViewY, self.option.width, count * CPPBHeight);
    if (self.option.isShowAnimation) {
        [self promitBoxRemoveFromSuperViewWithValues:CPPBAnimation_appear isDelegate:NO];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.images != nil) {
        cell.imageView.image = CPPBImage(self.images[indexPath.row]);
    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    return indexPath.row == (self.titles.count - 1) ? CPPBHeight + 1 : CPPBHeight;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [self selectorPreCell];
    [self promitBoxRemoveFromSuperViewWithValues:CPPBAnimation_disappear isDelegate:YES];
}
...
- (CGPoint)getPromitBoxPostionWithClipView:(UIView *)clipView{
    ...
    CGFloat disForRight = (CPPBScreenWidth - centerP.x - (width_self * .5));
