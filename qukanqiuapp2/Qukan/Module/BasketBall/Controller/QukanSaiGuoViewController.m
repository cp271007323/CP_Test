...
@interface QukanSaiGuoViewController ()<UITableViewDelegate, UITableViewDataSource, DZNEmptyDataSetDelegate, DZNEmptyDataSetSource>
...
@implementation QukanSaiGuoViewController
...
- (void)requestDataFromSever{
    ...
    @weakify(self)
    [[[kApiManager QukanFormPKWithTime:[NSString stringWithFormat:@"%ld",self.int_fixDays] xtype:[NSString stringWithFormat:@"%zd",self.int_xType] list:self.Qukan_leagueIds] takeUntil:self.rac_willDeallocSignal] subscribeNext:^(id  _Nullable x) {
        @strongify(self)
        KHideHUD
