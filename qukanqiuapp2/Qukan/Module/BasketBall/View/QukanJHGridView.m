...
@interface QukanJHGridView()<UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate>
...
@implementation QukanJHGridView
...
//            titleView.layer.borderColor = [[UIColor groupTableViewBackgroundColor] CGColor];
    ...
    for (long i = 0; i < titleNum; i++) {
        ...
        [tableView registerClass:[QukanJHGridCell class] forCellReuseIdentifier:@"QukanJHGridCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    NSString *cellId = @"QukanJHGridCell";
    QukanJHGridCell *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    long index = indexPath.row;
    cell.contentLab.text = [[_objects objectAtIndex:index] valueForKey:[_tags objectAtIndex:tableView.tag]];
    [self applyAlignmentTypeFor:cell.contentLab];
    GridIndex gridIndex;
    ...
    cell.contentLab.textColor = [self textColorForGridAtGridIndex:gridIndex];
    [cell.contentLab setFont:[self fontSizeForGridAtGridIndex:gridIndex]];
    cell.botLine.backgroundColor = [self botLineBackgroundColor];
...
- (UIColor *)botLineBackgroundColor {
    if ([_delegate respondsToSelector:@selector(botLineBackgroundColor)]) {
        return [_delegate botLineBackgroundColor];
    }else{
