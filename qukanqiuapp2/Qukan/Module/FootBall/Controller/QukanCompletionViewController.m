...
@interface QukanCompletionViewController ()<UITableViewDelegate, UITableViewDataSource, DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
...
@implementation QukanCompletionViewController
...
- (void)viewDidLoad {
    ...
    self.Qukan_dateView.Qukan_didSelectItemBlock = ^(NSInteger dateType) {
        @strongify(self)
        if (dateType == self.Qukan_fixDays) {
            return;
        }
        self.Qukan_fixDays = dateType;
...
//            [self Qukan_requestData];
//        }else {
////            [self.Qukan_myTableView scrollToRow:0 inSection:0 atScrollPosition:UITableViewScrollPositionTop animated:NO];
...
//        [self.Qukan_dataArray removeAllObjects];
    ...
    @weakify(self)
    [[[kApiManager QukanmatchInfosWithType:@"2" andLeague_ids:self.Qukan_leagueIds addOffset_day:[NSString stringWithFormat:@"%ld", self.Qukan_fixDays] andAll:[NSNumber numberWithBool:YES] addIshot:@""] takeUntil:self.rac_willDeallocSignal] subscribeNext:^(id  _Nullable x) {
        @strongify(self)
        KHideHUD
         self.bool_shouldShowEmpty = YES;
        [self.refreshBtn endAnimation];
...
//        if (self.clickDate) {
//            matchs.count ? [self.Qukan_myTableView scrollToRow:0 inSection:0 atScrollPosition:UITableViewScrollPositionTop animated:NO] : nil;
//        }
...
- (void)dataSourceDealWith:(id)response dateDay:(NSInteger)day {
    ...
    if ([response[@"finished_match"] isKindOfClass:[NSArray class]]) {
        NSArray *nowMatch = response[@"finished_match"];
        for (NSDictionary *d in nowMatch) {
            @autoreleasepool {
                ...
                QukanContentModel.contentType = 3;
...
- (QukanDateView *)Qukan_dateView {
    if (!_Qukan_dateView) {
        ...
        [_Qukan_dateView Qukan_setFirstSevenDaysData];
    }
