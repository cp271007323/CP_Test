...
- (NSLayoutConstraint *)TTConstraintForAttribute:(NSLayoutAttribute)attribute theView:(UIView *)view
{
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"firstItem = %@ && firstAttribute = %d ", view, attribute];
    NSArray *predicatedArray = [view.constraints filteredArrayUsingPredicate:predicate];
...
- (void)animateIgniteShow:(CGFloat)radius duration:(NSTimeInterval)duration delay:(NSTimeInterval)delay
{
    ...
    NSLayoutConstraint *heightConstraint = [self TTConstraintForAttribute:NSLayoutAttributeHeight theView:self];
...
- (void)animateIgniteHide:(NSTimeInterval)duration delay:(NSTimeInterval)delay
{
    ...
    for (UIView *dotView in @[self.dotFirst, self.dotSecond]) {
        ...
        NSLayoutConstraint *heightConstraint = [self  TTConstraintForAttribute:NSLayoutAttributeHeight theView:dotView];
        heightConstraint.constant = 0;
        
        NSLayoutConstraint *widthConstraint = [self TTConstraintForAttribute:NSLayoutAttributeWidth theView:dotView];
